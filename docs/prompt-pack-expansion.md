...
PixWeave now provides a deterministic internal prompt-pack workflow for the roadmap's configurable prompt-pack milestone. A pack declares `prompt-pack/v1`, a semantic version, a format template, and a non-empty variable matrix. `python3 -m pixweave prompt-pack examples/prompt-pack.json` validates and expands every combination into an atomic JSON manifest.
